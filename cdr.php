...
function _moduleContent(&$smarty, $module_name)
{
    ...
    // SÃ³lo el administrador puede consultar con $extension == NULL
    ...
    // Cadenas estÃ¡ticas en la plantilla
    $smarty->assign(array(
        "Filter"    =>  _tr("Filter"),
    ));
    ...
    // ParÃ¡metros base y validaciÃ³n de parÃ¡metros
    ...
    if (!$oFilterForm->validateForm($paramFiltro)) {
        ...
        unset($_POST['delete']);    // Se aborta el intento de borrar CDRs, si habÃ­a uno.
    }

    // Tradudir fechas a formato ISO para comparaciÃ³n y para API de CDRs.
...
// AbstracciÃ³n de privilegio por mÃ³dulo hasta implementar (Elastix bug #1100).
// ParÃ¡metro $module se usarÃ¡ en un futuro al implementar paloACL::hasModulePrivilege().
function hasModulePrivilege($user, $module, $privilege)
{
    ...
    return ($isAdmin && in_array($privilege, array(
        'reportany',    // Â¿EstÃ¡ autorizado el usuario a ver la informaciÃ³n de todos los demÃ¡s?
        'deleteany',    // Â¿EstÃ¡ autorizado el usuario a borrar CDRs?
    )));
}
